import ContactForm from "@/components/about/contact-form";
import Image from "next/image";
import { fetchAboutContent, TAbout } from "@/lib/data";
import { Separator } from "@/components/ui/separator";
import ContactInfo from "@/components/about/contact-info";


export default async function AboutPage() {
  const data = await fetchAboutContent();
  const { h1, description, image, imageAlt, formFooter } = data as TAbout;

  return (
    <div className="grid grid-cols-[1fr_1fr] gap-14 min-h-screen">
      {/* Left side */}
      <section className="flex flex-col pt-[12vh] pr-[8vh] border-r border-primary">
        <div className="flex gap-4 -mb-20 z-10">
          <h1 className="text-[5rem] leading-[6vw] font-instrument-serif uppercase">{h1}</h1>
          <p className=" px-4 font-varta">{description}</p>
        </div>
        {/* Image */}
        <div className="relative w-full h-[600px]">
          <Image
            src={image}
            alt={imageAlt}
            fill
            className="object-cover "
          />
        </div>
      </section>

      {/* Right side */}
      <section className="flex flex-col justify-start max-w-4xl gap-12 pt-[11vh]">
        <h1 className=" text-[5rem] leading-[4vw] font-instrument-serif uppercase">
          {formFooter}
        </h1>
        <div className="flex flex-col gap-12 justify-between w-full">
          <ContactForm />
          <ContactInfo />
        </div>

      </section>
    </div>
  );
}
